...
class Loan extends CI_Controller
{
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (empty($id)) {
                    ...
                    if (!empty($emvalue->status)) {
                        echo "Already you have a loan. Please pay installment first";
                    } else {
        ...
        if ($this->session->userdata('user_login_access') != false) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (empty($id)) {
                    ...
                    $status = ($installno == '1') ? 'Done' : 'Granted';
                    ...
                    $success = $this->loan_model->update_LoanData($loanid, $data_update);
                    echo "Successfully Added";
                } else {
                    $data_update = array(
                        'emp_id' => $em_id,
                    ...
                    $success = $this->loan_model->update_LoanInstallData($id, $data_update);
                    echo "Successfully Updated";
                }
            }
        } else {
    ...
    // NEW: Print loan per employee
    public function Print_Loan($emp_id)
    {
        if ($this->session->userdata('user_login_access') != false) {
            $data['employee'] = $this->employee_model->getEmployeeById($emp_id);
            $data['loans'] = $this->loan_model->getLoansByEmployee($emp_id);
            $this->load->view('backend/print_loan', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
