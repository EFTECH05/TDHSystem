...
public function Save() { 
    if($this->session->userdata('user_login_access') != False) {     
        ...
        $address = $this->input->post('address'); // <-- define $address here
 ...
 public function Reset_Password_Hr() {
    ...
    if($this->session->userdata('user_login_access') != false) {
        ...
        // Fixed: match the exact value from the database ('SUPER ADMIN')
        if($this->session->userdata('user_type') != 'SUPER ADMIN') {
            echo "Access Denied. Only Super Admin can update passwords.";
            return; // Stop execution
        }

        // Get the employee ID and new passwords from the form
        $id = $this->input->post('emid');
        $onep = $this->input->post('new1'); // New password
        $twop = $this->input->post('new2'); // Confirm password

        // Check if the new password and confirm password match
        if($onep == $twop){
            // Prepare data to update the password
            $data = array(
                'em_password'=> sha1($onep) // Hash the password using SHA1
            );

            // Call the model function to update the password in the database
            ...
            if($success){
                ...
                // Something went wrong while updating in the database
...
// If user is not logged in, redirect to the homepage hhdhddjjdjdj sorry 
